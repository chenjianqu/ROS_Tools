...
#define DDRES_SINGLE 0x1
#define DDRES_PREUPDATE 0x2
#define DDRES_REFPOS 0x4
#define DDRES_UPDATE 0x8
#define DDRES_FIX 0x10
#define DDRES_PREHOLD 0x20
#define DDRES_HOLDFIXPOS2AMB 0x40
#define DDRES_LAMBDAFIXPOS2AMB 0x80
...
#define lock0(f)     EnterCriticalSection(f)
#define unlock0(f)   LeaveCriticalSection(f)
#define FILEPATHSEP '\\'
#else
#define thread_t    pthread_t
#define lock_t      pthread_mutex_t
#define initlock(f) pthread_mutex_init(f,NULL)
#define lock0(f)     pthread_mutex_lock(f)
#define unlock0(f)   pthread_mutex_unlock(f)
#define FILEPATHSEP '/'
#endif
#define RB_STATUE_NONE      0x1
#define RB_STATUE_HASNOPOS  0x2
#define RB_STATUE_CHANGE    0x4
#define RB_STATUE_NORMAL    0x8
#define RB_STATUE_CHANGEOFFSET    0x10

#define IsZeo(d,Ep) ((d<Ep?1:0)&&(d>-Ep?1:0))
#ifndef FALSE 
#define FALSE  0
#endif
#ifndef TRUE 
#define TRUE  1
#endif
#define MAXSATNFQ    (MAXSAT *NFREQ)

//#define SLIPDETECT_TDREF  0x1000//参考星有周跳
#define SLIPDETECT_TDVOK    0x2000//TD残差有效，存在三差观测量
//#define SLIPDETECT_TDLSQV 0x4000 //可以用于计算LSQ的三差观测值
#define SLIPDETECT_TDLSQVOK 0x8000//经LSQ补偿后的残差探测处周跳
//#define SLIPDETECT_TDLSQV 0x4000
#define SLIPDETECT_DDVOK    0x200

#define TDSTATUSLSQV   0x1
#define TDSTATUSBASECHECK   0x2

#define KFCTL_NONE 0x0
#define KFCTL_INITALL 0x1
#define KFCTL_INITPOS 0x2
#define KFCTL_INITVEL 0x4
#define KFCTL_INITAMB 0x8
#define KFCTL_RESETAMB 0x10
#define KFCTL_RUN 0x20
...
typedef struct {        /* solution type */
    ...
    double RollPitchYaw[3];//deg
    double llh[3];//[0]:纬度  [1]:经度  [2]高程
    double utm[3];
} sol_t;
...
typedef struct 
{
    int week;
    double second; 
    double posllh[3]; //llh deg纬度  经度  高程
    double velEnu[3];//Enu  m/s
    double PRY[3]; //pitch,raw, yaw  (deg)
    int ref_scae;
    int Q[2];
}Ref_status_t;
...
typedef struct 
{
    int n;
    int nmax;
    Ref_status_t *pRef_status;
}Ref_statusd_t;
#define WEEKINSECOND (7*24*3600)
...
typedef struct 
{
    //u_int8_t lsqcandidateok[NFREQ];
    Amb_t dd_statepoch;
    Amb_t ZdAmb[2][2];//[0:上个历元，1：上上个历元][0:移动站  1,基站]
    int status[NFREQ];
    Amb_t tdv;
}TdCycleSlip_t;
...
typedef struct 
{
    ...
    uint8_t f;
    uint16_t index;
    double Hdd_statepoch[3];
    double vdd_statepoch;
    #ifdef PLAYBACKMODE
    double refvdd_statepoch;
    #endif
}TdCycleSlip_LSQ_t;
typedef struct 
{
    uint8_t holdflag[MAXSAT][NFREQ];
    double zdamb[MAXSAT][NFREQ];
    int zdambsum;
    uint8_t zdambsyssum[NSYS][NFREQ];
    double HoldFixsimga;
    sol_t sol;
    double dop[4];
    int fixsat;
...
typedef struct {        /* satellite status type */
    ...
    Amb_t UptAmb;
    Amb_t ZdAmb[2][2];//[0:上个历元，1：上上个历元][0:移动站  1,基站]
    TdCycleSlip_t TdCycleSlip;
#ifdef PLAYBACKMODE
    Amb_t refZdAmb[2];//移动站参考位置计算的非差[0:当前历元（可以理解成当前历元）， 1上个历元]
#endif
...
typedef struct 
{
    ...
    double mstdfixvp[2];
    double mstdfixvl[2];
    int satfix_SF[NSYS*NFREQ] ;
    int fixsat;
} ddres_Info_t;
typedef struct 
{
    obsd_t obsd[2][MAXSAT];
}LastObsInfo_t;
...
#ifdef PLAYBACKMODE
    Ref_status_t Ref_status;
#endif
    double second;
    double basesecond;
    tdInfo_t tdInfo;
    kfCtl_t kfCtl;
    PreHoldFix_t PreHoldFix;   
    PreHoldFix_t lambdaFix;
    ddres_Info_t ddres_Info[4]; 
    LastObsInfo_t  LastObsInfo;
} rtk_t;
...
EXPORT double norm_rms(const double *a, int n);
EXPORT int sumint(const int *a, int n);
...
EXPORT void imatfprint(const int A[], int n, int m, int p, int q, FILE *fp);
...
EXPORT int timeiszero(gtime_t t);
EXPORT int timeisInt(gtime_t t, double Eps);
EXPORT int DoubleIsInt(double second, double Eps);
...
EXPORT void traceimat(int level, const int *A, int n, int m, int p, int q);
...
EXPORT uint32_t getbits_index(const int buff);
...
EXPORT int decode_frame(const uint8_t *buff, eph_t *eph, alm_t *alm,
                        double *ion, double *utc, double *p);
...
// int showmsg_(1,const char *format,...);
 void settspan(gtime_t ts, gtime_t te);
 void settime(gtime_t time);

 int showmsg_(int a,char *format,...);
class Ctrace_t;
 void InitGnssData(obs_t *pobs, int *piobsu, int *piobsr, rtk_t *pRtkCtl,
                   sta_approx_t *psta_approx, int *psta_approx_cout, Ctrace_t *ctrace);
 int inputobs(obsd_t *obs, int solq, const prcopt_t *popt);
 int procpos_exc(FILE *fp, FILE *fptm, const prcopt_t *popt, const solopt_t *sopt,
                 rtk_t *rtk, int mode, obsd_t *obs_ptr, int nobs);
 //int GetRefPos(const int week, const double second, Ref_status_t *pRef_status, const char *str);
 int validobs(int i, int j, int f, int nf, const double *y);


 void rtk_amb_offset(rtk_t *rtk, int sat, int f, double amb);
 void SetInsFeedBack(const Ins_FeedBack_t*pIns_FeedBack);
int test_sys(int sys, int m);

//template<class T>

//template<class T>
extern void cal_eachotherdis(const double*a, double*out, int n);


template<typename T>
int caldata_mean_std(const T *data, const int n, double *mean, double*std);

template<typename T>
int caldata_mean_stdabs(const T *data, const int n, double *mean, double*std);

//template<typename T>
void Multi(const double *left,int f1, int f2, const double *right,  int s1, int s2, double alpha, double beta,  double *result);
void caldata_rms(const double *data, int n, double *rms);
/* test satellite system (m=0:GPS/SBS,1:GLO,2:GAL,3:BDS,4:QZS,5:IRN) ---------*/
int test_sys(int sys, int m);
int sys2m(int sys);

// template<typename T>
// int caldata_median(const T *data, const int n, T *median);

// template<typename T>
// int caldata_median_distance(const T *data, T *distance, int n,double *median_dian);
