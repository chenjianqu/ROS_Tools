...
extern double satwavelen(int sat, int frq, const nav_t *nav)
{
    ...
    if (sys==SYS_GLO) {
        ...
        else if (frq==3) return CLIGHT/FREQL6; /* E6 */
        else if (frq==5) return CLIGHT/FREQE5ab; /* E5ab */
    }
    else { /* GPS,QZS */
        if      (frq==0) return CLIGHT/FREQL1; /* L1 */
        else if (frq==1) return CLIGHT/FREQL2; /* L2 */
        else if (frq==2) return CLIGHT/FREQE5ab; /* L5 */
        else if (frq==3) return CLIGHT/FREQL6; /* L6/LEX */
...
extern uint32_t getbits_index(const int buff)
{
    uint32_t bits=0;
    int i;
    for (i=0;i<32;i++) 
...
extern double norm_rms(const double *a, int n)
{
    return n>1?sqrt(dot(a,a,n)/(double)n):0;
}
extern int sumint(const int *a, int n)
{
    int sum = 0;
    for(int i =0; i<n; i++)
    {
...
extern void imatfprint(const int A[], int n, int m, int p, int q, FILE *fp)
{
    int i,j;
...
extern int timeiszero(gtime_t t)
{
    if(fabs(t.time)<1e-9 &&fabs(t.sec)<1e-9)  return 1;
    else return 0;   
}
extern int timeisInt(gtime_t t, double Eps)
{
    double t0 = t.time+t.sec;
    double diff = t0 -round(t0);
    if(fabs(diff)<Eps)  return 1;
    else return 0;   
}
extern int DoubleIsInt(double second, double Eps)
{
    double diff = second -round(second);
    if(fabs(diff)<Eps)  return 1;
    else return 0;  
}
...
extern int readpcv(const char *file, pcvs_t *pcvs)
{
    ...
    if (!(ext=strrchr(const_cast<char*>(file),'.'))) ext="";
...
extern void traceimat(int level, const int *A, int n, int m, int p, int q)
{
    if (!fp_trace||level>level_trace) return;
    imatfprint(A,n,m,p,q,fp_trace); fflush(fp_trace);
}
...
extern int expath(const char *path, char *paths[], int nmax)
{
    ...
    if ((p=strrchr(const_cast<char*>(path),'/'))||(p=strrchr(const_cast<char*>(path),'\\'))) {
